...
const CartTotals: React.FC<CartTotalsProps> = ({
  ...
  const { currency_code, total, tax_total, item_subtotal, shipping_subtotal } =
    totals

  const displaySubtotal =
    subtotalOverride !== undefined ? subtotalOverride : (item_subtotal ?? 0)
  const displayTotal =
    totalOverride !== undefined ? totalOverride : (total ?? 0)
